...
class Install:
    ...
    def _install_cmake(self):
        ...
        print(
            "CMake version is less than 3.20, start installation ({})".format(
                cmake_name
            )
        )
        self._cmd(
            "wget -t 10 {} -P {}".format(
                "https://cmake.org/files/v3.22/{}.tar.gz".format(cmake_name),
                self._dowload_path,
            )
        )
    ...
    def _clone_dds(self):
        ...
        if not os.path.exists(os.path.join(self._dowload_path, dds_name)):
            self._cmd(
                "wget -t 10 {} -P {}".format(
                    "https://apollo-system.cdn.bcebos.com/archive/6.0/{}".format(
                        dds_name
                    ),
                    self._dowload_path,
                ),
    ...
    def _install_dds2(self):
        ...
        self._cmd(
            "patch -p1 < {}".format(
                os.path.join(self._current_path, "scripts/Fast-CDR_v2.2.2.patch")
            )
        )
        self._cmd("mkdir -p build")
        os.chdir("build")
        ...
        os.chdir("build")
        self._cmd(
            "cmake -DCMAKE_INSTALL_PREFIX={} -DCMAKE_PREFIX_PATH={} -DBUILD_SHARED_LIBS=ON ..".format(
                self._install_prefix,self._install_prefix
            )
        )
        self._cmd("make install -j$(($(nproc) - 1))")
        os.chdir(self._current_path)
    ...
    def _unpack_bvar(self):
        ...
        self._cmd(
            "wget -t 10 {} -O {}".format(
                download_url, os.path.join(self._dowload_path, "bvar.deb")
            )
        )
        self._cmd(
            "dpkg -x {} {}".format(
                os.path.join(self._dowload_path, "bvar.deb"),
                os.path.join(self._dowload_path, "bvar"),
            )
        )
        self._cmd(
            "cp -r {}* {}".format(
                os.path.join(self._dowload_path, "bvar/usr/local/"),
                self._install_prefix,
            )
        )
        return None
...
def parse_config():
    ...
    parser.add_argument(
        "--platform", type=str, default=platform.machine(), help="platform"
    )
    parser.add_argument(
